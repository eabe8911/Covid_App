...
try {
	...
	while (($csv = fgetcsv($file_open, 10000, ",")) !== false) {
        ...
        //cname 中文名
        //$cname = trim($csv[24]);
        if(str_replace(" ", "", $csv[24]) == 'NA'){
            $cname = "";
        }elseif(str_replace(" ", "", $csv[24]) == 'N/A'){
            $cname = "";
        }elseif(str_replace(" ", "", $csv[24]) == '無'){
            $cname = "";
        }else{
            $cname = str_replace("-", "", str_replace(" ", "", $csv[24]));
            $cname = str_replace("　", "", $csv[24]);
        }

        //fname first name , lname last name 英文名(新版合在一起，統一寫在fname)
        //$fname = trim($csv[26]);
        if(str_replace(" ", "", $csv[26]) == 'NA'){
            $fname = "";
        }elseif(str_replace(" ", "", $csv[26]) == 'N/A'){
            $fname = "";
        }elseif(str_replace(" ", "", $csv[26]) == '無'){
            $fname = "";
        }else{
        ...
        if (!empty(trim($csv[42]))) {
            $sendname = str_replace(" ", "", $csv[42]);
            echo($csv[42].$sendname.'<br>');
        } else{
            $dd = explode("(", $csv[9]);
            //echo $dd[0]; echo '<br>';
            $apdat =  date('Y-m-d', strtotime($dd[0]));
        ...
        //testreason 自費篩檢原因
        if (substr($csv[7],0,1) == '1') {
            $testreason = '1';
        } elseif (substr($csv[7],0,1) == '2') {
            $testreason = '2';
        } elseif (substr($csv[7],0,1) == '3') {
            $testreason = '3';
        } elseif (substr($csv[7],0,1) == '4') {
            $testreason = '4';
        } elseif (substr($csv[7],0,1) == '5') {
            $testreason = '5';
        } elseif (substr($csv[7],0,1) == '6') {
            $testreason = '6';
        } else {
            $testreason = '7';
        }
        ...
        //hicardno 健保卡號
        $hicardno = "";
        if(str_replace(" ", "", $csv[17]) == 'NA'){
            $hicardno = "";
        }elseif(str_replace(" ", "", $csv[17]) == 'N/A'){
            $hicardno = "";
        }elseif(str_replace(" ", "", $csv[17]) == '無'){
            $hicardno = "";
        }else{
        ...
        if(substr($csv[14],0,1) == "3"){
            $twrpturgency = "hiurgent";
        }elseif(substr($csv[14],0,1) == "2"){
            $twrpturgency = "urgent";
        }elseif(substr($csv[14],0,1) == "1"){
            $twrpturgency = "normal";
        }
