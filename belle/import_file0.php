...
if (isset($_POST["submit_file"])) {
	...
	while (($csv = fgetcsv($file_open, 6000, ",")) !== false) {
		...
		if ($myline >= 1) {
            ...
            //mtpid 台胞證
			if (!empty(trim($csv[43]))) {
                        	$mtpid = strtoupper(str_replace(" ", "", $csv[43]));
                        } else {
				$mtpid = "";
			}
			
			//日本報告國籍欄位
			if (!empty(trim($csv[45]))) {
				$nationality = strtoupper(str_replace(" ", "", $csv[45]));
			} else {
				$nationality = "";
			}
			...
			if ($stmt = mysqli_prepare($conn, $sql)) {
				...
				if (mysqli_stmt_execute($stmt)) {
					...
					if (mysqli_stmt_num_rows($stmt) == 0) {
						...
						// mobile 手機
						$mobile = str_replace(" ", "", $csv[30]);
                                                //$mobile = str_replace(" ", "", $csv[26]);//olive

						//uemail 顧客信箱
						$uemail = str_replace(" ", "", $csv[32]);
						...
						// telephone 市話
						$telephone = str_replace(" ", "", $csv[31]);
                                                //$telephone = str_replace(" ", "", $csv[27]); //olive

						//address2 地址
						$address2 = trim($csv[34]);
                                                //$address2 = trim($csv[30]); //olive

						//testtype 檢測類型
						if (str_replace(" ", "", $csv[10]) == '抗原快篩/Ag_TC') {
                                                //if (str_replace(" ", "", $csv[8]) == '抗原快篩 / Ag_TC ;') { //olive
							$testtype = 1;
						} elseif (str_replace(" ", "", $csv[10]) == '核酸檢測/PCR;') {
						...
						//testreason 自費篩檢原因
						if (trim($csv[9]) == '因旅外親屬事故或重病等緊急特殊因素入境他國家/地區須檢附檢驗證明之民眾。需檢附(1)申請表；(2)申請原因相關文件， 如電子機票、購票證明或訂票紀錄等') {
						//if (trim($csv[7]) == '因旅外親屬事故或重病等緊急特殊因素入境他國家/地區須檢附檢驗證明之民眾。需檢附(1)申請表；(2)申請原因相關文件， 如電子機票、購票證明或訂票紀錄等') {
							$testreason = '1';
						} elseif (trim($csv[9]) == '因工作因素須檢附檢驗證明之民眾。需檢附(1)申請表；(2)工作證明文件，如職員證、工作簽證、出差通知書、電子機票、購票證明或訂票紀錄等') {
						//} elseif (trim($csv[7]) == '因工作因素須檢附檢驗證明之民眾。需檢附(1)申請表；(2)工作證明文件，如職員證、工作簽證、出差通知書、電子機票、購票證明或訂票紀錄等') {
							$testreason = '2';
						} elseif (trim($csv[9]) == '短期商務人士。需檢附(1)申請表；(2)申請原因相關文件（如： 在臺行程表或防疫計畫書等') {
						//} elseif (trim($csv[7]) == '短期商務人士。需檢附(1)申請表；(2)申請原因相關文件（如： 在臺行程表或防疫計畫書等') {
							$testreason = '3';
						} elseif (trim($csv[9]) == '出國求學須檢附檢驗證明之民眾。需檢附(1)申請表；(2)就學證明文件，如學生證、學生簽證、入學通知書、電子機票、購票證明或訂票紀錄等') {
						//} elseif (trim($csv[7]) == '出國求學須檢附檢驗證明之民眾。需檢附(1)申請表；(2)就學證明文件，如學生證、學生簽證、入學通知書、電子機票、購票證明或訂票紀錄等') {
							$testreason = '4';
						} elseif (trim($csv[9]) == '外國或中國大陸、香港、澳門人士出境。需檢附(1)申請表；(2) 護照、入臺許可證、電子機票、購票證明或訂票紀錄等') {
						//} elseif (trim($csv[7]) == '外國或中國大陸、香港、澳門人士出境。需檢附(1)申請表；(2) 護照、入臺許可證、電子機票、購票證明或訂票紀錄等') {
							$testreason = '5';
						} elseif (trim($csv[9]) == '相關出境適用對象之眷屬。需檢附(1)申請表；(2)身分證及相關出境適用對象之關係證明文件，如戶口名簿、戶籍謄本、適用對象之工作、就學證明等文件等') {
						...
						if (trim($csv[33]) == '需要，請幫我郵寄') {
						//if (trim($csv[29]) == '需要，請幫我郵寄') { //olive
							$mailrpt = "1";
						} elseif (trim($csv[33]) == '我會親自到場領取紙本報告') {
						...
						//if (str_replace(" ", "", $csv[31]) == "是/Yes") { //olive
							...
							$mtpid = str_replace(" ", "", $csv[38]);
                                                        //$mtpid = str_replace(" ", "", $csv[34]); //olive
							$cmobile = str_replace(" ", "", $csv[39]);
							//$cmobile = str_replace(" ", "", $csv[35]); //olive
							if (str_replace(" ", "", $csv[40]) == '') {
							//if (str_replace(" ", "", $csv[36]) == '') { //olive
								$xmemail = $uemail;
							} else {
								$xmemail = str_replace(" ", "", $csv[40]);
						...
						preg_match($cheinese_pattern, $csv[13], $matches);
						...
						$cheinese_pattern = "/\p{Han}+/u";
						preg_match($cheinese_pattern, $csv[37], $matches);
					...
					// sql row count >0, if the data did not been used. The row can delete.
						...
						//if (trim($csv[29]) == '需要，請幫我郵寄') { //olive
							$mailrpt = "1";
						} elseif (trim($csv[33]) == '不需要，我會親自到場領取紙本報告') {
						//} elseif (trim($csv[29]) == '不需要，我會親自到場領取紙本報告') { //olive
							$mailrpt = "2";
						} else {
							$mailrpt = "3";
