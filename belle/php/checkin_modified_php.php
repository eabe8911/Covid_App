...
$result_msg = '1';
// Connect to local db
$conn = mysqli_connect($HostNPort, $username, $password);
...
$fname=$mobile=$sex=$uemail=$sendname=$testreason=$hicardno=$mtpid=$xmappoint="";
$testtype = '0';
$page=$page_count=0;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"])) {
    ...
    if (empty($sampleid_err)) {
        ...
        apdat=?,testtype=?,cname=?,dob=?,sex=?,uemail=?,sendname=?,testreason=?,fname=?,lname=?,type=?,
        nationality=?,mtpid=?,twrpturgency=?,xmappoint=?,tdat=?
        where uuid=?";
        if ($stmt = mysqli_prepare($conn, $sql2)) {
            ...
            $p1,$p2,$p3,$p4,$p5,$p6,$p7,$p8,$p9,$p10,$p11,$p12,$p13,$p14,$p15,$p16,$p17,$p19,$p20,$p21,$p22,$p23,$p24
        );
        ...
        if(empty($_POST["tdat"])){
            ...
            $p23 = strtotime($_POST["tdat"]);
            $p23 = date("Y-m-d H:i:s", $p23);
        }
        $p24 = trim($_POST["uuid"]);
        ...
        twrpturgency={$p21},xmappoint={$p22},tdat={$p23}
        where uuid={$p24}";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["checkin_x"])) {
    ...
    if (empty($sampleid_err)) {
        ...
        $p15 = trim($_POST["fname"]);
        if($check2 == 1){
            // save sampleid and report date 有修改資料
            $sql2 = "update covid_trans set sampleid1= ?,sampleid2=? ,
                passportid=?, userid=?, mobile=?, hicardno=?,
            ...
            if ($stmt = mysqli_prepare($conn, $sql2)) {
                ...
                $sql_comment = $_SESSION["username"] .
                    ": update covid_trans set sampleid1={$p1}, sampleid2={$sampleid2} ,
                    passportid={$p3}, userid={$p4}, mobile={$p5}, hicardno={$p6},
            ...
            // 沒有修改資料
            $sql2 = "update covid_trans set tdat=? where uuid=?";
            if ($stmt = mysqli_prepare($conn, $sql2)) {
                //echo "New record created successfully";
                mysqli_stmt_bind_param( $stmt, "si", $p1, $p2 );
            
                // Set parameters
                date_default_timezone_set("Asia/Taipei");
                $p1 =(string)date("Y-m-d H:i:s");
                $p2 = trim($_POST["uuid"]);
                $sampleid2 = trim($_POST["sampleid2"]);;

                $sql_comment = $_SESSION["username"].": update covid_trans set tdat={$p1} where uuid={$p2}";
                write_sql($sql_comment);
            }
        }
        // Attempt to execute the prepared statement
        if (mysqli_stmt_execute($stmt)) {
            $save_result = $p9.'<br>'.$p15.'<br>'.$p4.'<br>'.$p3.'<br>'.$sampleid2.'<br>報到成功';
            write_sql($save_result);
            $result_msg = '4';
        } else {
            $save_result = "Error: " . $sql2 . "<br>" . mysqli_error($conn);
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["search"] )){
	...
	if (empty(trim($_POST["pcrid_uuid"])) && empty(trim($_POST["userid_passid"])) && empty(trim($_POST["mobile"]))){
		$input_err = "Please enter 'PCD ID', 'Passport NO.', 'User ID.' or mobile no.";
	} else {
		if(empty(trim($_POST["pcrid_uuid"]))) {
            ...
            if(strlen($_POST["pcrid_uuid"]) > 9){
                // PCR ID
                $sampleid = $_POST["pcrid_uuid"];
                if(substr($sampleid,0,1)=='Q'){
                    //PCR
                    $condition = "sampleid2='".$sampleid."'";
                }else{
                    //快篩
                    $condition = "sampleid1='".$sampleid."'";
                }
                ...
                FROM covid_trans WHERE ".$condition;
            }else{
                // UUID
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(empty($user_data)){
                    ...
                    if(count($user_data) == 0){                    
                        ...
                        if(empty($user_data[0]["sampleid1"])){
                            $pcrid_uuid     = $user_data[0]["sampleid2"];
                        }else{
                            $pcrid_uuid     = $user_data[0]["sampleid1"];
                        }
                       ...
                       if ($positive->QueryUser($user_data[0],date('Y-m-d'))){
                            $result_msg = '8';
                       }
