...
function generate_result($sql_element, $sampleid)
{
    ...
    $sql_general = "SELECT cname, userid, sex, fname, lname, dob, mobile, uemail, address2,address1 from libodb.covid_trans 
    ...
    $sql_test = "SELECT apdat, tdat, passportid, hicardno, testtype,sampleid1, sampleid2 from libodb.covid_trans 
    WHERE {$sql_element} ='{$sampleid}' limit 1;";
    ...
    from libodb.covid_trans WHERE {$sql_element} ='{$sampleid}' limit 1;";
    ...
    if ($testtype == "1") {
        if ($fpdfflag == "Y") {
            ...
            echo '<br><h5 style="justify-content: center; display: flex;color:#FF0000">請再次確認檢測結果</h5>';
        ...
        if ($pcrpdfflag == "Y") {
            ...
            if ($nationality!=""){
                echo '<br><h5 style="justify-content: center; display: flex;color:#556B2F">日本入境檢驗證明</h5>';
