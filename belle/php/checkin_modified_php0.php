...
$sql2=$stmt="";
$nationality=$twrpturgency=$per_type=$ename=$tdat=$apdat=$cname=$dob=$lname=$fname=$mobile=$sex=$uemail=$testtype=$sendname=$testreason=$hicardno="";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"]))
{
    ...
    if(empty($sampleid_err)){
        ...
        apdat=?,testtype=?,cname=?,dob=?,sex=?,uemail=?,sendname=?,testreason=?,fname=?,lname=?,type=?,tdat=?,nationality=?
        where uuid=?";
        ...
        testreason={$p14},fname={$p15},lname={$p16},type={$p17},tdat={$p18},nationality={$p19}
        where uuid={$p20}";
    ...
    if(empty($sampleid_err)){
        ...
        tdat=?,type=?,nationality=? where uuid=?";
        if ($stmt = mysqli_prepare($conn, $sql2)) {
            //echo "New record created successfully";
        ...
        tdat={$p17},type={$p18},nationality={$p19} where uuid={$p20}";
        write_sql($sql_comment);
        }
        // Attempt to execute the prepared statement
        if(mysqli_stmt_execute($stmt)){
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["search"] )){
    ...
    if(empty($input_err)){
        ...
        $sql = "SELECT uuid,userid, passportid,cname,lname,fname,mobile,uemail,sex,dob,apdat,tdat,sampleid1,sampleid2,testtype,sendname,testreason,hicardno,`type`,twrpturgency,nationality
        ...
        if($stmt = mysqli_prepare($conn, $sql)){
            ...
            $sql_comment=$_SESSION["username"].": SELECT uuid,userid, passportid,cname,lname,fname,mobile,uemail,sex,dob,apdat,tdat,sampleid1,sampleid2,testtype,sendname,testreason,hicardno,`type`,twrpturgency,nationality
            FROM  covid_trans WHERE 1=1 
            and apdat >= (select curdate()-20 from dual) 
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt,$uuid, $userid, $passportid,$cname,$lname,$fname,$mobile,$uemail,$sex,$dob,$apdat,$tdat,$sampleid1,$sampleid2,$testtype,$sendname,$testreason,$hicardno,$per_type,$twrpturgency,$nationality);
