...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $file_name = "/var/www/html/xls_reports/" . "covid_data_report" . date("y-m-d") . ".xlsx";
    ...
    $sql = "SELECT testtype,type,ctzn,userid,passportid,sex,cname,fname,lname
                                ...
                                ,hicardno,mobilerpt,mailrpt,hbrpt,hbrptyear,cloudrpt,cloudrptyear,nihrpt
                                    FROM covid_trans 
                                    WHERE SUBSTR(sampleid2,1,2) <> 'QH' AND DATE_FORMAT(tdat, '%Y-%m-%d') ='".$bdaymonth."'" ;
...
<script>
    $(document).ready(function () {
         ...
         var today = time.getFullYear() + "-" + (month) + "-" + (time);
         $('#bdaymonth').val(today);
     })
 </script>
