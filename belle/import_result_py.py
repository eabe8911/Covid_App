...
def query_SQL_data(sample_name):
    ...
    sample_name_full =re.match(r"PC|NTC|^Q[0-9]{9}|^QL[0-9]{9}|^QH[0-9]{9}|^F[0-9]{9}", sample_id)
    if sample_name_full!=None:
        ...
        if len(sample_id)!=15:
            ...
            sample_id=sample_id
    cmd = "SELECT pcrtest FROM covid_trans WHERE sampleid2 = '" + sample_id + "'"
    cursor.execute(cmd)
    #conn.commit()
...
def upload_SQL(sample_name, result_all):
    sample_id=str(sample_name)
    sample_name_full =re.match(r"PC|NTC|^Q[0-9]{9}|^QL[0-9]{8}|^QH[0-9]{9}|^F[0-9]{9}", sample_id)
    if sample_name_full!=None:
        sample_id=sample_name_full[0]
    else:
        if len(sample_id)!=15:
    ...
    if result_all.split(',')[-2].upper() == "":
        if result_all.split(',')[-1].upper() in ['POSITIVE', 'NEGATIVE']:
            ...
            cmd = "UPDATE covid_trans SET pcrtest = '" + result_all.split(',')[-1] + "',rdat='" + now + "' WHERE sampleid2 = '" + sample_id + "'"
        else:
            cmd = "UPDATE covid_trans SET xlspcrtest2 = 'N', pcrtest = '" + result_all.split(',')[-1] + "' WHERE sampleid2 = '" + sample_id + "'"
    elif result_all.split(',')[-2].upper() == result_all.split(',')[-1].upper() and result_all.split(',')[-1].upper() in ['POSITIVE', 'NEGATIVE']:
        cmd = "UPDATE covid_trans SET xlspcrtest2 = 'Y' WHERE sampleid2 = '" + sample_id + "'"
    else:
        cmd = "UPDATE covid_trans SET xlspcrtest2 = 'N', pcrtest = '', rdat=NULL WHERE sampleid2 = '" + sample_id + "'"
    cursor.execute(cmd)
    conn.commit()
    # return cmd
