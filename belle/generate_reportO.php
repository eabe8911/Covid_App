...
$save_result = '';
$nationality = $stmt = $testid = $uuid = $mtpid = $userid = $sex = $cname = $fname = $lname = $ename = $dob = $mobile = $uemail = $ftest = $pcrtest = $address2 = $passportid = $sampleid1 = $sampleid2 = $residentpermit = $tdat = $rdat = $sendname = $frptflag = $qrptflag = $residentpermitid = $vuser1 = $vuser2 = $testtype = '';
$testid_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Generate_Report"])) {
	...
	system('python3 /var/www/html/belle/update_report2.py ' . $testid);
	$sql_comment = $_SESSION["username"] . ": " . 'python3 /var/www/html/belle/update_report2.py ' . $testid;
	write_sql($sql_comment, "BeanCode");

	// 要做出不同國籍的報告
	system('python3 /var/www/html/belle/write_pdf_report.py ' . $testid);
	$sql_comment = $_SESSION["username"] . ": " . 'python3 /var/www/html/belle/write_pdf_report.py' . $testid;
	write_sql($sql_comment, "BeanCode");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Search"])) {
	...
	if ($testid_err == "") {
		...
		$pattern = "/^Q[0-9]{9}|^QH[0-9]{9}|^F[0-9]{9}|^QL[0-9]{9}/"; //olive add QL NO.
		...
		if (empty($matches)) {
			...
			if ($testid[0] == 'F') {
				$sql = "SELECT uuid, userid, sex, cname, fname, lname, dob, mobile, uemail, ftest, pcrtest,
							...
							frptflag, qrptflag, hicardno  ,vuser1,vuser2,testtype,nationality,mtpid,xmrpturgency
						FROM covid_trans WHERE 1=1 and sampleid1=? ";
			} else {
				$sql = "SELECT uuid, userid, sex, cname, fname, lname, dob, mobile, uemail, ftest, pcrtest,
							address2, passportid, sampleid1, sampleid2, residentpermit, tdat, rdat, sendname,
							frptflag, qrptflag, hicardno ,vuser1,vuser2,testtype,nationality,mtpid,xmrpturgency
						FROM covid_trans WHERE 1=1 and sampleid2=? ";
			}
				...
				<div class="row g-3" id="read">
					...
					<div class="col-md-6">
						...
						<div id="PointMsgEmail"></div>
                                      ...
                                      <div class="col-md-3">
						<label>台胞證號碼</label>
						<input type="text" name="mtpid" id="mtpid" class="form-control " value="<?php echo $mtpid; ?>">
					</div>
					...
					<!-- <div class="col-md-3">
						<label>採樣方式</label>
						<input type="text" name="xmrpturgency" id="xmrpturgency" class="form-control " value="<?php echo $xmrpturgency; ?>">
