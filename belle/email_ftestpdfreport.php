...
function send_ftest_email($sampleid){
    ...
    // $sql = "select uuid,userid,passportid,uemail,cname,fname,lname,dob,apdat,tdat, sampleid1 from covid_trans where 1=1
            ...
            if(mysqli_stmt_num_rows($stmt) > 0){ 
                ...
                $mail->addAddress($uemail, 'ToEmail');
                    ...
                    if(!empty($sampleid1)){
                        ...
                        $send_file = "/var/www/html/pdf_reports/{$valid_date}/{$sampleid1}.pdf";
                    }
                ...
                if (!empty($send_file)){
                    ...
                    if(!$mail->send()) {
                        ...
                        $sql2 = "update covid_trans set frptflag= ? where uuid = ?";
                        if ($stmt2 = mysqli_prepare($conn, $sql2)) {
                            ...
                            $sql_comment=$_SESSION["username"].": update covid_trans set frptflag= {$p1} where uuid = {$p2}";
                            write_sql($sql_comment, "BeanCode");
                        }
                    }
                }
