...
function send_pcrtest_email($sampleid){
    ...
    // $sql = "select uuid,userid,passportid,uemail,cname,fname,lname,dob,apdat,tdat,sampleid2 from covid_trans where 1=1
    ...
    #國籍
    $sql = "select uuid,userid,passportid,uemail,cname,fname,lname,dob,apdat,tdat,sampleid2,nationality from covid_trans where 1=1
                                                                            and sampleid2='{$sampleid}'
                                                                            and trim(pcrtest)<>'NA'
                                                                            and trim(pcrtest)<>''
                                                                            and trim(uemail)<>''";
            ...
            if(mysqli_stmt_num_rows($stmt) > 0){ 
                ...
                $mail->addAddress($uemail, 'ToEmail');
                    ...
                    if(!empty($sampleid2) && $nationality!=""){
                        ...
                        $send_file = "/var/www/html/pdf_reports/{$valid_date}/{$sampleid2}.pdf";
                        $send_file1 = "/var/www/html/pdf_reports/Japanese_report/{$valid_date}/{$sampleid2}_Japanese_report.pdf";
                ...
                if (!empty($send_file)){
                    ...
                    if(!$mail->send()) {
                        ...
                        $sql2 = "update covid_trans set qrptflag= ? where uuid = ?";
                        if ($stmt2 = mysqli_prepare($conn, $sql2)) {
                            ...
                            $sql_comment=$_SESSION["username"].": update covid_trans set qrptflag= {$p1} where uuid = {$p2}";
                            write_sql($sql_comment, "BeanCode");
