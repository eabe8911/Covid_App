...
try {
    ...
    while (($csv = fgetcsv($file_open, 10000, ",")) !== false) {
        ...
        //userid 身分證
        if (!empty(trim($csv[22]))) {
            $userid = strtoupper(str_replace(" ", "", $csv[22]));
            // echo("(1)");
        } else {
            if ($residentpermit == "Y") {
                $userid = str_replace("-", "", str_replace(" ", "", $csv[24]));
                // echo("(2)");
            } else {
                $userid = "";
                // echo("(3)");
        ...
        //passportid 護照
        if (!empty(trim($csv[32]))) {
            if (str_replace(" ", "", $csv[32]) == 'NA') {
                $passportid = "";
            } elseif (str_replace(" ", "", $csv[32]) == 'N/A') {
                $passportid = "";
            } elseif (str_replace(" ", "", $csv[32]) == '無') {
                $passportid = "";
            } else {
                $passportid = strtoupper(str_replace(" ", "", $csv[32]));
            }
        } else {
            $passportid = "";
        }
        ...
        if (!empty(trim($csv[15]))) {
            $mtpid = strtoupper(str_replace(" ", "", $csv[15]));
        } else {
            $mtpid = "";
        }

        //日本報告國籍欄位
        if (!empty(trim($csv[17]))) {
            $nationality = strtoupper(str_replace(" ", "", $csv[17]));
        } else {
            $nationality = "";
        }

        //apdat 預約日期
        $apdat = date('Y-m-d', strtotime($csv[19]));
        ...
        if (str_replace(" ", "", $csv[30]) == 'NA') {
            ...
            $cname = str_replace("-", "", str_replace(" ", "", $csv[30]));
            $cname = str_replace("　", "", $csv[30]);
        }

        //fname first name , lname last name 英文名(新版合在一起，統一寫在fname)
        //$fname = trim($csv[26]);
        if (str_replace(" ", "", $csv[31]) == 'NA') {
            $fname = "";
        } elseif (str_replace(" ", "", $csv[31]) == 'N/A') {
            $fname = "";
        } elseif (str_replace(" ", "", $csv[31]) == '無') {
            $fname = "";
        } else {
        ...
        // mobile 手機
        $mobile = str_replace(" ", "", $csv[35]);

        //uemail 顧客信箱
        $uemail = str_replace(" ", "", $csv[37]);
        ...
        if (substr($csv[6], 0, 1) == '1') {
            ...
            $type = 2;
            $sendname = str_replace(" ", "", $csv[10]);
        }

        if (!empty(trim($csv[9]))) {
            $sendname = str_replace(" ", "", $csv[10]);
            echo ($csv[10] . $sendname . '<br>');
        } else {
            $dd = explode("(", $csv[19]);
            //echo $dd[0]; echo '<br>';
            $apdat = date('Y-m-d', strtotime($dd[0]));
        ...
        //公司抬頭
        $companytitle = trim($csv[11]);

        //address1 廢欄位 
        $address1 = "";

        // telephone 市話
        $telephone = str_replace(" ", "", $csv[36]);

        //address2 地址
        $address2 = trim($csv[39]);
        ...
        if (trim($csv[38]) == '需要，請幫我郵寄') {
            $mailrpt = "1";
        } elseif (trim($csv[38]) == '我會親自到場領取紙本報告') {
            $mailrpt = "2";
        } else {
            $mailrpt = "3";
        }
        ...
        //hicardno 健保卡號
        $hicardno = "";
        if (str_replace(" ", "", $csv[25]) == 'NA') {
            $hicardno = "";
        } elseif (str_replace(" ", "", $csv[25]) == 'N/A') {
            $hicardno = "";
        } elseif (str_replace(" ", "", $csv[25]) == '無') {
            $hicardno = "";
        } else {
            $hicardno = str_replace("-", "", str_replace(" ", "", $csv[25]));
        }
        ...
        if (substr($csv[41], 0, 1) == "1") {
            //$xmappoint = "Y";
            //預約日
            $dd = explode(" ", $csv[42]);
            //$dd = explode(" ", $csv[32]); //olive
            $xmapdat = date('Y-m-d', strtotime($dd[0]));
            $mtpid = str_replace(" ", "", $csv[15]);
            $cmobile = str_replace(" ", "", $csv[45]);
            if (str_replace(" ", "", $csv[46]) == '') {
                $xmemail = $uemail;
            } else {
                $xmemail = str_replace(" ", "", $csv[46]);
            }
        } else {
            //$xmappoint = "N";
        ...
        if (substr($csv[18], 0, 1) == "3") {
            $twrpturgency = "hiurgent";
        } elseif (substr($csv[18], 0, 1) == "4") {
            $pmflag = "Y";
            $twrpturgency = "hiurgent";
        } elseif (substr($csv[18], 0, 1) == "2") {
            $twrpturgency = "urgent";
        } elseif (substr($csv[18], 0, 1) == "1") {
            $twrpturgency = "normal";
        }
        ...
        //TODO 新增資料時，要匯入sampleid2，當天日期+流水號
        if ($apdat <> '') {
            $pcr = new Addpcrid();
            //TODO　如果substr($csv[18], 0, 1) != "4"　不是下午特急件，自動遞增編號
            if (substr($csv[18], 0, 1) != "4") {
                try {
                    // TODO: get pcr id
                    if (!$pcr->getPcrLastID($apdat)) {
                        $sampleid2 = $pcr->getPcrLastID($apdat);
                        ...
                        // if ($pcr->AddPcrid($payload) == false) {
                        //     throw new Exception($pcr->get_errorMessage(), 1);
                        // }
                    } else {
                        //TODO  to get getPcrLastID ($sampleid2)
                        $sampleid2 = $pcr->getPcrLastID($apdat);
                    }
                } catch (Exception $th) {
                    echo $th->getMessage();
            ...
            //TODO　如果substr($csv[18], 0, 1) != "4"　下午特急件，需編號9開頭
            else {
                try {
                    // TODO: get pcr id
                    if ($pcr->getAfternoonPcrLastID($apdat)) {
                        $sampleid2 = $pcr->getAfternoonPcrLastID($apdat);
                        // TODO: pcr id add 1
                        $payload1 = [
                            'sampleid2' => $sampleid2,
                            'apdat' => $apdat,
                        ...
                        //TODO  to get getAfternoonPcrLastID ($sampleid2)
                        $sampleid2 = $pcr->getAfternoonPcrLastID($apdat);
                    }
                } catch (Exception $th) {
                    echo $th->getMessage();
        ...
        mailrpt, hbrpt, hbrptyear, cloudrpt, cloudrptyear, nihrpt, xmemail, approval,  address1, hiflag, fpdfflag, pcrpdfflag, nationality, xlspcrtest2, companytitle, sampleid2, pmflag 
        ) VALUES (
...
<body>
    <div>
        <input style="margin:1em;" type="button" class="btn btn-secondary" onclick="history.back()"
            value="回到上一頁"></input>
        <input style="margin:1em;" type="button" class="btn btn-secondary" onclick="window.location.href='menu.php'"
            value="回首頁"></input>
        <input style="margin:1em;" type="button" class="btn btn-secondary"
            onclick="window.location.href='menu_version1.html'" value="回舊版首頁"></input>
