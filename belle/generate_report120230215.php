...
$stmt2=$stmt=$stmt3=$stmt4=$stmt5=$stmt6=$stmt7=$stmt8="";
$uuid=$apdat=$userid=$passportid=$cname=$lname=$fname=$mobile=$uemail=$sex="";
$dob=$ftest=$pcrtest=$vuser1=$testtype=$frptflag=$vuser2=$sendname=$total_sample="";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Search"])){
   ...
   //060921 add limit 1 to ensure always select the last row of uuid 
    if($tdat_err == "")
	{
		// $sql91 = "SELECT COUNT(*) FROM covid_trans WHERE 1=1 and apdat = tdat";
		// if($stmt91 = mysqli_prepare($conn, $sq9l))
		// {
		// 	mysqli_stmt_bind_param($stmt91, "s", $p1);
		// 	$p91 = $apdat."%";
		// 	mysqli_stmt_execute($stmt91);
		// 	mysqli_stmt_bind_result($stmt91, $total_sample);
		// 	mysqli_stmt_fetch($stmt91);
		// 	mysqli_stmt_close($stmt91);
		// 	//echo $total_sample;
		// }
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" style='display:inline'>
			...
			<div class="form-group" style='display:inline'>
				<!-- <label>總報名人數</label><br>
                <input type="text" name="allpeople"  value="<?php echo $total_sample; ?>" readonly tabindex=4><br> -->
                <label>總報到人數</label><br>
                <input type="text" name="sampleno"  value="<?php echo $total_sample; ?>" readonly tabindex=5><br>
				<label>完成檢測人數</label><br>
                <input type="text" name="tested"  value="<?php echo $tested; ?>" readonly tabindex=6><br>
				<label>需發快篩報告人數</label><br>
                <input type="text" name="n_ftest"  value="<?php echo $n_ftest; ?>" readonly tabindex=7><br>
				<label>已產生快篩報告人數</label><br>
                <input type="text" name="freported"  value="<?php echo $freported; ?>" readonly tabindex=8><br>
				<label>需發PCR報告人數</label><br>
                <input type="text" name="n_pcrtest"  value="<?php echo $n_pcrtest; ?>" readonly tabindex=9><br>
				<label>已產生PCR報告人數</label><br>
                <input type="text" name="pcrreported"  value="<?php echo $pcrreported; ?>" readonly tabindex=10><br>
				<label>未產生報告人數</label><br>
                <input type="text" name="unreported"  value="<?php echo $unreported; ?>" readonly tabindex=11><br>
				<label>未寄出急件報告</label><br>
                <input type="text" name="unreported"  value="<?php echo $emailnum; ?>" readonly tabindex=12><br>
                <br>
				<br>
			</div> 
        </form>
