...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Upload"]))
{
    ...
    if (empty(trim($_POST["vuser2"]))|| empty(trim($_POST["vuser2"])) || empty(trim($_POST["sendname"]))){
        ...
        $sql5 = "update covid_trans set vuser1= ?, vuser2= ?,qrptflag='C', pcrtest='negative', rdat=(select NOW() from dual) where 1=1
																			and tdat LIKE ? 
																			and (testtype = '2' or testtype = '3') 
																			and (pcrtest='' or pcrtest is null or pcrtest='NA') 
																			and sendname=?";
        ...
        $sql_comment=$_SESSION["username"]."_version1: update covid_trans set vuser1= {$p1}, vuser2= {$p2},qrptflag='C', pcrtest='negative', rdat=(select NOW() from dual) where 1=1
        and tdat LIKE {$p3} 
        and (testtype = '2' or testtype = '3') 
        and (pcrtest='' or pcrtest is null or pcrtest='NA') 
        and sendname={$p4}";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Search"])){
   ...
   //060921 add limit 1 to ensure always select the last row of uuid 
    if($vuser1_err == "" && $vuser2_err == "" && $sendname_err == "" && $tdat_err == "")
	{
		...
		$sql = "SELECT COUNT(*) FROM covid_trans WHERE 1=1
												and tdat LIKE ? 
												and (testtype = '2' or testtype = '3') 
												and sendname=?";
		...
		$sql4 = "SELECT COUNT(*) FROM covid_trans WHERE 1=1
												and tdat LIKE ? 
												and (testtype='2' or testtype = '3')
												and (pcrtest='' or pcrtest is null or pcrtest='NA') 
												and sendname=?";
