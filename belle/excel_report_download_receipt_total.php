...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $file_name = "/var/www/html/xls_reports/" . "libobio_covid_receipt_total_" . $bdaymonth . ".xlsx";



    $sql = "SELECT sampleid2,receiptid,cname,fname,sendname,twrpturgency,payflag,companytitle
    FROM covid_trans
    WHERE apdat ='" . $bdaymonth . "' AND tdat is not null AND payflag <> '4'
    ORDER BY sampleid2";
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_num_rows($stmt) > 0) {
                ...
                $alignment = $sheet->getStyle('F6:F500')->getAlignment();
                $alignment->setHorizontal(Alignment::HORIZONTAL_LEFT);

                $alignment1 = $sheet->getStyle('C6:D500')->getAlignment();
                $alignment1->setHorizontal(Alignment::HORIZONTAL_LEFT);
                ...
                while (mysqli_stmt_fetch($stmt)) {
                    ...
                    $sheet->setCellValueByColumnAndRow(4, $row, $companytitle); //買受人(抬頭)
                    ...
                    $newvalue = 1 * intval($price); //將price轉換成數字
