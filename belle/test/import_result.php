                    ...
                    if (confirm(textpositive) == true) {
                        $.ajax({
                            ...
                            success: function(msg) {
                                ...
                                } else if (msg["checknumber"] == 0) {
                                    if (alert('上傳成功!')) {} else window.location.reload();
                                } else if (msg["checknumber"] == 2) {
                                    alert(data);
                                    alert("請重新檢查檢體編號後再次上傳");
                                }
                                // console.log(msg);
        ...
        <div class="col py-3" style="margin:1em;">
            <div>
                <form style="display:inline;" id="start">
                    <h3> 請將 PCR 結果 xls 或 xlsx 檔上傳。</h3>
                    <p>若檢體編號有誤或該檢體編號不存在於資料庫，上傳結果便會取消。</p>
                    <input type="text" id="login_MT" value="<?php echo $_SESSION["username"] ?>" disabled hidden>
                    <input class="form-control" name="file" type="file" id="upload_file" style="width:18rem;"> <br>
                    <br>
                    <div class="col-auto">
