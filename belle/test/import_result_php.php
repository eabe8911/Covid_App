...
function send_file()
{
    ...
    if ($uploadOk == 0) {
        ...
        $checkresult = checkby_python("import_file", $path);
        if ($checkresult == 0) {
            echo "檢查通過。";
            $sql_comment = $_SESSION["username"] . ": " . "檢查通過。";
            write_sql($sql_comment);
            // call_python("import_file", $path);
        } else {
            echo "檢查未通過，上傳終止。";
            $sql_comment = $_SESSION["username"] . ": " . "檢查未通過，上傳終止。";
            write_sql($sql_comment);
        }
    }
}
...
function upload_vuser($sampleid)
{
    ...
    $sql_general1 = "SELECT uuid,vuser1,vuser2,xlspcrtest2 from libodb.covid_test 
    ...
    while ($row_general1 = $result_general1->fetch_assoc()) {
        ...
        $sql2 = "update covid_test set vuser1=?,vuser2=?,qrptflag=? where uuid=?";
        if ($stmt2 = mysqli_prepare($conn, $sql2)) {
            ...
            $sql_comment = $_SESSION["username"] . ": update covid_test set vuser1={$p1},vuser2={$p2},qrptflag={$p3} where uuid={$p4}";
            write_sql($sql_comment);
        }
...
function generate_result($sampleid)
{
    ...
    $sql_general = "SELECT pcrtest,rdat,xlspcrtest2,qrptflag from libodb.covid_test 
    WHERE sampleid2 ='{$sampleid}' limit 1;";
    ...
    if ($result_general->num_rows > 0) {
        ...
        while ($row_general = $result_general->fetch_assoc()) {
            ...
            $sql_comment = $_SESSION["username"] . ": SELECT pcrtest={$row_general["pcrtest"]},rdat={$row_general["rdat"]},xlspcrtest2={$row_general["xlspcrtest2"]},qrptflag={$row_general["qrptflag"]} from libodb.covid_test 
            WHERE sampleid2 ='{$sampleid}' limit 1;";
            write_sql($sql_comment);
        }
    } else {
...
function checkby_python($arg, $arg2)
{
    $checkresult = 0;
    ...
    //     if ($line[1] == "L") {
    //     } else {
    //         $sql_general = "SELECT sampleid2,tdat,qrptflag from libodb.covid_test
    //         WHERE sampleid2 ='{$line}' limit 1;";
    //         $result_general = $conn->query($sql_general);
    //         $row_cnt = $result_general->num_rows;

    //         if ($row_cnt == 0) {
    //             echo $line . " 此檢體編號並不存在於資料庫中，上傳終止。<br>";
    //             $checkresult = 1;
    //             $sql_comment = $_SESSION["username"] . ":" . $line . " 此檢體編號並不存在於資料庫中，上傳終止。<br>";
    //             write_sql($sql_comment);
    //         }
    ...
    //                 } else if ($i == 1) {
    //                     if ($item == NULL) {
    //                         echo $line . " 此檢體編號並無報到時間，上傳終止。<br>";
    //                         $checkresult = 1;
    //                         $sql_comment = $_SESSION["username"] . ":" . $line . " 此檢體編號並無報到時間，上傳終止。<br>";
    //                         write_sql($sql_comment);
    //                     }
    //                 } else if ($i == 2) {
    //                     if ($item == "C") {
    //                         echo $line . " 此檢體編號檢驗結果已上傳過，上傳終止。<br>";
    //                         $checkresult = 1;
    //                         $sql_comment = $_SESSION["username"] . ":" . $line . " 此檢體編號檢驗結果已上傳過，上傳終止。<br>";
    //                         write_sql($sql_comment);
    //                     }
    //                 }
    //                 $i += 1;
