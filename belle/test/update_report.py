...
if testid[0] == 'Q' or (re.match(r"[0-9]",testid[0])!=None):
    cmd = ("SELECT uuid,cname,fname,lname,sex,dob,mobile,userid,residentpermit,passportid,uemail,tdat,type,rdat,ftest,pcrtest,vuser2,hicardno,sendname,mtpid " 
            "FROM covid_test WHERE sampleid2 = '" + testid + "'")
elif testid[0] == 'F':
    cmd = ("SELECT uuid,cname,fname,lname,sex,dob,mobile,userid,residentpermit,passportid,uemail,tdat,type,rdat,ftest,pcrtest,vuser2,hicardno,sendname,mtpid " 
            "FROM covid_test WHERE sampleid1 = '" + testid + "'")
else:
...
if cmd != '':
    ...
    if testid[0] == 'Q' or (re.match(r"[0-9]",testid[0])!=None):
        ...
        if tdate in os.listdir('/var/www/html/belle/test/pdf_reports/'):
            pass
        else:
            os.system('mkdir /var/www/html/belle/test/pdf_reports/' + tdate)
            os.system('chmod 777 /var/www/html/belle/test/pdf_reports/' + tdate)
        if name + '.docx' in os.listdir('/var/www/html/belle/test/pdf_reports/' + tdate + '/'):
            os.system('rm /var/www/html/belle/test/pdf_reports/' + tdate + '/' + name + '.docx')
        else:
            pass
        if name + '.pdf' in os.listdir('/var/www/html/belle/test/pdf_reports/' + tdate + '/'):
            os.system('rm /var/www/html/belle/test/pdf_reports/' + tdate + '/' + name + '.pdf')
        else:
            pass
        if testid + '.pdf' in os.listdir('/var/www/html/belle/test/pdf_reports/' + tdate + '/'):
            os.system('rm /var/www/html/belle/test/pdf_reports/' + tdate + '/' + testid + '.pdf')
        else:
            pass
        doc.save('/var/www/html/belle/test/pdf_reports/' + tdate + '/' + name + '.docx')
        os.system('chmod 777 /var/www/html/belle/test/pdf_reports/' + tdate + '/' + name + '.docx')
...
path = '/var/www/html/belle/test/pdf_reports/' + tdate + '/'
os.system('libreoffice --headless --convert-to pdf:writer_pdf_Export --outdir ' + path + ' /var/www/html/belle/test/pdf_reports/' + tdate + '/' + name + '.docx 1>/dev/null 2>&1')
