                ...
                if (mysqli_connect_errno()) {
                    ...
                    for ($i = 0; $i < count($nameArr); $i++) {
                        if (($nameArr[$i] != "")) { /*not allowing empty values and the row which has been removed.*/
                            $sql = "update covid_test set vuser1= '{$vuser1resultArr[$i]}', vuser2= '{$vuser2resultArr[$i]}',frptflag='C', ftest='{$ftestresultArr[$i]}', rdat='{$rdatresultArr[$i]}' WHERE uuid='{$uuidresultArr[$i]}';";
...
function uploadresult($dateinput)
{
    ...
    $sql_general = "SELECT sampleid1,ftest,rdat,vuser1,vuser2,uuid FROM libodb.covid_test 
