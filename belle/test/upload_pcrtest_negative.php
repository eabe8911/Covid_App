...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Upload"]))
{
    ...
    if (empty(trim($_POST["vuser2"]))|| empty(trim($_POST["vuser2"])) || empty(trim($_POST["sendname"]))){
        ...
        $sql5 = "update covid_test set vuser1= ?, vuser2= ?,qrptflag='C', pcrtest='negative', rdat=(select NOW() from dual) where 1=1
																			and tdat LIKE ? 
        ...
        $sql_comment=$_SESSION["username"]."_version1: update covid_test set vuser1= {$p1}, vuser2= {$p2},qrptflag='C', pcrtest='negative', rdat=(select NOW() from dual) where 1=1
        and tdat LIKE {$p3} 
        and (testtype = '2' or testtype = '3') 
        and (pcrtest='' or pcrtest is null or pcrtest='NA') 
        and sendname={$p4}";
