...
$result_msg = '1';
// Connect to local db
// $conn = mysqli_connect("192.168.2.115","root","password");
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"]))
{
    ...
    if(empty($sampleid_err)){
        ...
        tdat=?,type=?,nationality=?,mtpid=?,twrpturgency=? where uuid=?";
        if ($stmt = mysqli_prepare($conn, $sql2)) {
            //echo "New record created successfully";
        ...
        tdat={$p17},type={$p18},nationality={$p19},mtpid={$p20},twrpturgency={$p21} where uuid={$p22}";
        write_sql($sql_comment);
        }
        // Attempt to execute the prepared statement
        if(mysqli_stmt_execute($stmt)){
