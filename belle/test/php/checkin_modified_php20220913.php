...
$fname=$mobile=$sex=$uemail=$testtype=$sendname=$testreason=$hicardno=$mtpid="";
$page=$page_count=0;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"])) {
    ...
    if (empty($sampleid_err)) {
        ...
        apdat=?,testtype=?,cname=?,dob=?,sex=?,uemail=?,sendname=?,testreason=?,fname=?,lname=?,type=?,nationality=?,mtpid=?,twrpturgency=?
        where uuid=?";
        if ($stmt = mysqli_prepare($conn, $sql2)) {
            ...
            $p1,$p2,$p3,$p4,$p5,$p6,$p7,$p8,$p9,$p10,$p11,$p12,$p13,$p14,$p15,$p16,$p17,$p19,$p20,$p21,$p22
        );
        ...
        testreason={$p14},fname={$p15},lname={$p16},type={$p17},nationality={$p19},mtpid={$p20},twrpturgency={$p21},
        where uuid={$p22}";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["search"] )){
	...
	if (empty(trim($_POST["uuid"])) && empty(trim($_POST["userid"])) && empty(trim($_POST["passportid"])) && empty(trim($_POST["mobile"]))){
		$input_err = "Please enter 'ID', 'Passport NO.', 'Mobile NO.'.";
	} else {
		if(empty(trim($_POST["uuid"]))) {
            //$id_err = "E";
            $uuid="NA";
		} else {
			$uuid = trim($_POST["uuid"]);
            $sql = "SELECT uuid,userid, passportid,cname,lname,fname,mobile,uemail,
                sex,dob,apdat,tdat,sampleid1,sampleid2,testtype,sendname,testreason,
