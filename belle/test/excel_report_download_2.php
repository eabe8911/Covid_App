...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $file_name = "/var/www/html/xls_reports/" . "Clinic_covid_" . date("y-m-d") . ".xlsx";
    ...
    $sql = "SELECT twrpturgency,userid,passportid,sex,cname,
            ...
            FROM covid_trans 
            WHERE SUBSTR(sampleid2,1,2) = 'QH' AND DATE_FORMAT(tdat, '%Y-%m') ='".$bdaymonth."'
            ORDER BY twrpturgency,tdat" ;
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_num_rows($stmt) > 0) {
                ...
                while (mysqli_stmt_fetch($stmt)) {
                    ...
                    //     $company = "個人";
                    // }
                    if($twrpturgency == 'normal') {
                        $normal_num =  $normal_num + 1;
                        $sheet->setCellValueByColumnAndRow(1, $row, $twrpturgency);
                        //$sheet->getStyle()->getFont()->getColor()->setARGB('969696');
                    } else {
                        $urgent_num =  $urgent_num + 1;
                        $sheet->setCellValueByColumnAndRow(1, $row, $twrpturgency);
                        //$sheet->getStyle()->getFont()->getColor()->setARGB('FF9696');
                    }
                ...
                $sheet->setCellValueByColumnAndRow(1, $row+2, '急件='.$urgent_num);
                $sheet->setCellValueByColumnAndRow(1, $row+3, '一般件='.$normal_num);
