...
def find_sample_info(PCRid):
    ...
    try:
        ...
        ##################################### Need to be modified
        cmd = "SELECT sex,fname,dob,pcrtest,passportid,vuser2,tdat,rdat,nationality FROM covid_test WHERE sampleid2='" + PCRid + "'"
        #####################################
...
if nationality!="":
    ...
    #English name
    can.drawString(4.5*cm, 23*cm, fname)
    #PassportID
    ...
    if tdate in os.listdir('/var/www/html/belle/test/pdf_reports/Japanese_report/'):
        pass
    else:
        os.system('mkdir /var/www/html/belle/test/pdf_reports/Japanese_report/' + tdate )
        os.system('chmod 777 /var/www/html/belle/test/pdf_reports/Japanese_report/' + tdate )

    outputStream = open("/var/www/html/belle/test/pdf_reports/Japanese_report/{}/{}_Japanese_report.pdf".format(tdate ,pcrid), "wb")
    output.write(outputStream)
    outputStream.close()
    os.system('chmod 777 /var/www/html/belle/test/pdf_reports/Japanese_report/{}/{}_Japanese_report.pdf'.format(tdate ,pcrid))
    print("<script>alert('入境日本證明報告位於後台伺服器 /var/www/html/belle/test/pdf_reports/Japanese_report/{}/{}_Japanese_report.pdf 路徑');</script>".format(tdate ,pcrid) )
    #####################################
