...
try {
	...
	while (($csv = fgetcsv($file_open, 6000, ",")) !== false) {
        ...
        // print_r($csv);die();
        if (!empty(trim($csv[7]))) {
            $userid = strtoupper(str_replace(" ", "", $csv[7]));
        } else {
            if(!empty(trim($csv[8]))) {
                $userid = str_replace("-", "", str_replace(" ", "", $csv[8]));
            }
            else{
            $userid = "";
            }	
        ...
        //apdat 預約日期
        $apdat =  date('Y-m-d', strtotime($csv[3]));
        ...
        // telephone 市話
        $telephone = str_replace(" ", "", $csv[15]);

        //address2 地址
        $address2 = trim($csv[16]);
        ...
        testtype, ctzn, passportid, cdcflag, sampleid2, residentpermit, mtpid, payflag, xmappoint, tdat, twrpturgency, rdat, hicardno, xmemail, 
        approval, address1, hiflag, fpdfflag, pcrpdfflag, nationality, xlspcrtest2
        ) VALUES (
