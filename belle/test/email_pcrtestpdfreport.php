...
// if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['send_email_pcrtest']))
// {
//         $value=$_POST['send_email_pcrtest'];
//         $sampleid = trim(str_replace(", Send E-mail", "", $value));
//         echo $sampleid;
//         send_pcrtest_email($sampleid);
// }

$value=$_POST['pcrsampleid_post'];
$sampleid = trim(str_replace(", Send E-mail", "", $value));
// echo $sampleid;
$message_back=send_pcrtest_email($sampleid);
...
function send_pcrtest_email($sampleid){
    ...
    // $sql = "select uuid,userid,passportid,uemail,cname,fname,lname,dob,apdat,tdat,sampleid2 from covid_test where 1=1
    //                                                                         and sampleid2='{$sampleid}'
    //                                                                         and trim(pcrtest)<>'NA'
    //                                                                         and trim(pcrtest)<>''
    //                                                                         and trim(uemail)<>''
    //                                                                         and lower(qrptflag)='c'";
    $sql = "select uuid,userid,passportid,uemail,cname,fname,lname,dob,apdat,tdat,sampleid2,nationality from covid_test where 1=1
                                                                            and sampleid2='{$sampleid}'
                                                                            and trim(pcrtest)<>'NA'
                                                                            and trim(pcrtest)<>''
                                                                            and trim(uemail)<>''";
    ...
    if($stmt = mysqli_prepare($conn, $sql)){                               
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_stmt_num_rows($stmt) > 0){ 
                ...
                mysqli_stmt_bind_result($stmt,$uuid,$userid,$passportid,$uemail,$cname,$fname,$lname,$dob,$apdat,$tdat,$sampleid2,$nationality);
                ...
                $mail->addAddress($uemail, 'ToEmail');
                    ...
                    //英文版
                        ...
                        $body=$cname."Dear ".$fname.",<BR> <BR>Here is your qPCR report on ".explode(":",$tdat)[0].":".explode(":",$tdat)[1].".<br>";
                        
                        
                        $body=$body."<br><br>Regards,<br>";
                        $body=$body."LIHPAO Medical Laboratory<br>";
                        $mail->Body    = $body;
                    ...
                    // search if the 身份證.pdf or 護照.pdf 存在
                    //if (!empty($userid) && empty($passportid) ){
                    //  $send_file = "/var/www/html/pdf_reports/".$userid."--".$apdat.".pdf";
                        //echo $send_file;
                    //}elseif (!empty($passportid)&& empty($userid)) {
                    //  $send_file = "/var/www/html/pdf_reports/".$passportid."-".$apdat.".pdf";
                    //}elseif (!empty($passportid)&& !empty($userid)) {
                        //$send_file = "/var/www/html/pdf_reports/".$userid."-".$passportid."-".$apdat.".pdf";
                    //}
                    // attach send_files, 檔案不存在，則檔名清空不發信
                    
                    if(!empty($sampleid2) && $nationality!=""){
                        $valid_date = date('Y-m-d', strtotime($tdat));
                        $send_file = "/var/www/html/belle/test/pdf_reports/{$valid_date}/{$sampleid2}.pdf";
                        $send_file1 = "/var/www/html/belle/test/pdf_reports/Japanese_report/{$valid_date}/{$sampleid2}_Japanese_report.pdf";
                    }else if(!empty($sampleid2)){
                        $valid_date = date('Y-m-d', strtotime($tdat));
                        $send_file = "/var/www/html/belle/test/pdf_reports/{$valid_date}/{$sampleid2}.pdf";
                        $send_file1 = "";
                    }else{
                    ...
                    if (file_exists($send_file) && file_exists($send_file1)){
                        //echo "ya got this";
                        // $file_name="{$sampleid}_{$final_name}_報告覆核完成版.pdf";
                        $mail->AddAttachment($send_file);
                        $mail->AddAttachment($send_file1);
                        // echo "YES";
                    }else if (file_exists($send_file)){
                        $mail->AddAttachment($send_file);
                    }else{
                        $send_file="";
                        $send_file1="";
                ...
                if (!empty($send_file)){
                    ...
                    if(!$mail->send()) {
                        ...
                        $message_back=$message_back. $userid.' '.$sampleid2.' '.$cname.' on '.$tdat.' Message send ';

                        $sql_comment=$_SESSION["username"].": ".$userid.' '.$sampleid2.' '.$cname.' on '.$tdat.' Message send';
                        ...
                        $sql2 = "update covid_test set qrptflag= ? where uuid = ?";
                        if ($stmt2 = mysqli_prepare($conn, $sql2)) {
                            ...
                            $sql_comment=$_SESSION["username"].": update covid_test set qrptflag= {$p1} where uuid = {$p2}";
                            write_sql($sql_comment, "BeanCode");
                        }
                    }
                }
