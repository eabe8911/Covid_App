...
function generate_sql_element($array){
    ...
    return [$sfdat,$efdat,$userid,$sex,$cname,$fname,$lname,$dob,$mobile,$uemail,$apdat,$ftest,$pcrtest,$smsflag,$emailflag,$type,$telephone,$address2,$testtype,$ctzn,$passportid,
...
if (isset($_POST["submit_excel"])) {
    ...
    for ($x = 0; $x <$dataLength; $x++) {
        ...
        if (array_key_exists('姓名', $array)&& $array['姓名']!=null){
            ...
            if (trim($userid) != "") {
                $sql = "SELECT uuid,userid,sampleid1,sampleid2 FROM covid_test WHERE 1=1
                                                                                        and    userid = ? 
                                                                                        and    apdat=?";
            } elseif (trim($passportid) != "") {
                $sql = "SELECT uuid,passportid,sampleid1,sampleid2 FROM covid_test WHERE 1=1
                                                                                        and    passportid = ? 
                                                                                        and    apdat=?";
            }
            // 3
                    ...
                    //6
                        ...
                        while (mysqli_stmt_fetch($stmt)) {
                            if (empty(trim($sampleid1)) && empty(trim($sampleid2))) {
                                $sql3 = "delete from covid_test where uuid=?";
